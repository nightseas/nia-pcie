// ---------------------------------------------------------------------------
// File        : example_core_pcie_split_width.v
// Description : The example core at a split TLP width. Holds the PCIe DMA
//               interface, the AXI master, the AXI RAM, the MSI-X AXI-Lite master
//               and the benchmark core, wired at the completion width presented.
...
module example_core_pcie_split_width #
(
    ...
    parameter TLP_CPL_DATA_WIDTH = TLP_DATA_WIDTH,
...
(
    ...
    input  wire [TLP_CPL_DATA_WIDTH-1:0]                 rx_req_tlp_data,
    input  wire [TLP_CPL_STRB_WIDTH-1:0]                 rx_req_tlp_strb,
    ...
    output wire [TLP_CPL_DATA_WIDTH-1:0]                 tx_cpl_tlp_data,
    output wire [TLP_CPL_STRB_WIDTH-1:0]                 tx_cpl_tlp_strb,
...
parameter AXI_DATA_WIDTH = TLP_CPL_DATA_WIDTH;
...
initial begin
    ...
    if (TLP_STRB_WIDTH*32 != TLP_DATA_WIDTH) begin
        $error("Error: requester TLP interface requires dword (32-bit) granularity: TLP_STRB_WIDTH*32 (%0d) != TLP_DATA_WIDTH (%0d) (instance %m)",
            TLP_STRB_WIDTH*32, TLP_DATA_WIDTH);
        $finish;
    end
    if (TLP_CPL_STRB_WIDTH*32 != TLP_CPL_DATA_WIDTH) begin
        $error("Error: completer TLP interface requires dword (32-bit) granularity: TLP_CPL_STRB_WIDTH*32 (%0d) != TLP_CPL_DATA_WIDTH (%0d) (instance %m)",
            TLP_CPL_STRB_WIDTH*32, TLP_CPL_DATA_WIDTH);
        $finish;
    end

    if ((TLP_CPL_DATA_WIDTH & (TLP_CPL_DATA_WIDTH-1)) != 0 ||
        TLP_CPL_DATA_WIDTH < 64 || TLP_CPL_DATA_WIDTH > 1024) begin
        $error("Error: TLP_CPL_DATA_WIDTH (%0d) must be a power of two in [64,1024] - it must equal the AXIS width of the CQ/CC interface that feeds this module, which is what pcie_us_if_cq.v:113 and pcie_us_if_cc.v:113 enforce, and pcie_us_if_{cq,cc}.v:91 bound the accepted family (instance %m)",
            TLP_CPL_DATA_WIDTH);
        $finish;
    end
end
...
wire [TLP_CPL_DATA_WIDTH-1:0]               ctrl_rx_req_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               ctrl_rx_req_tlp_strb;
...
wire [TLP_CPL_DATA_WIDTH-1:0]               ctrl_tx_cpl_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               ctrl_tx_cpl_tlp_strb;
...
wire [TLP_CPL_DATA_WIDTH-1:0]               ram_rx_req_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               ram_rx_req_tlp_strb;
...
wire [TLP_CPL_DATA_WIDTH-1:0]               ram_tx_cpl_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               ram_tx_cpl_tlp_strb;
...
wire [TLP_CPL_DATA_WIDTH-1:0]               msix_rx_req_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               msix_rx_req_tlp_strb;
...
wire [TLP_CPL_DATA_WIDTH-1:0]               msix_tx_cpl_tlp_data;
wire [TLP_CPL_STRB_WIDTH-1:0]               msix_tx_cpl_tlp_strb;
